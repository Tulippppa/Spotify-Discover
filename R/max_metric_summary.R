#' Finding the maximum for a specific music metric
#'
...
#' @param max_or_min Identification of desired max or min results (function(min_metric_summary) preferable for min outputs)
...
#'
#' @return A dataframe showing max value of music metric and the associated song from user playlist
#' @export
#'
#' @examples
#' # Here is an example dataframe for when you input the URI of Beyonce.
#' result <- max_metrics_summary(c('6vWDO969PvNqNYHIOW5v0m'), test = TRUE)
...
max_metrics_summary <- function(username, URI, access_token, max_or_min, ..., test = FALSE){
  ...
  if(test){return(max_metrics_data)} else {
    ...
    max_values <- playlist %>%
      ...
      # finding maximum value and selecting one max value
      summarize(title = track.name[value == max(value)][1],
                metric = metric[1],
                value = max(value)) %>%
    ...
    # returning dataframe with the max music value per metric with associated song title
    return(max_values)
}
